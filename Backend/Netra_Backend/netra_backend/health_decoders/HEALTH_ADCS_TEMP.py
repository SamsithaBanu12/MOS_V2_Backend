...
def HEALTH_ADCS_TEMP(hex_str):
    ...
    for idx in range(count):
        ...
        MCUTemperature = struct.unpack('<h', bytes.fromhex(seg[10:14]))[0]
       # Magnetometer_Temperature = struct.unpack('<h', bytes.fromhex(seg[14:18]))[0] * 0.1
       # Redundant_Magnetometer_Temperature = struct.unpack('<h', bytes.fromhex(seg[18:22]))[0] * 0.1
        Reserved1 = struct.unpack('<h', bytes.fromhex(seg[14:18]))[0]
        Reserved2 = struct.unpack('<h', bytes.fromhex(seg[18:22]))[0]
        segments.append({
            ...
            'MCU Temperature':         MCUTemperature,
              'Reserved1':              Reserved1,
                'Reserved2':              Reserved2
        })
    return segments
