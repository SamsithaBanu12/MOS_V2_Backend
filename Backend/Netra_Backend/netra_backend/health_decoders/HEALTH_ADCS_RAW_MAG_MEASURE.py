...
def HEALTH_ADCS_RAW_MAG_MEASURE(hex_str):
    ...
    for idx in range(count):
        ...
        raw_mag_measure_x = struct.unpack('<h', bytes.fromhex(seg[10:14]))[0]
        raw_mag_measure_y = struct.unpack('<h', bytes.fromhex(seg[14:18]))[0]
        raw_mag_measure_z = struct.unpack('<h', bytes.fromhex(seg[18:22]))[0]
        segments.append({
            ...
            'Raw_Mag_Measure_X':      raw_mag_measure_x,
            'Raw_Mag_Measure_Y':      raw_mag_measure_y,
            'Raw_Mag_Measure_Z':      raw_mag_measure_z
        })
    return segments
