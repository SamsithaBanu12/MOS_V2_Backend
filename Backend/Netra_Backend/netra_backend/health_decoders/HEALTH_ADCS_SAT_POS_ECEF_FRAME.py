...
def HEALTH_ADCS_SAT_POS_ECEF_FRAME(hex_str):
    ...
    for idx in range(count):
        ...
        sat_pos_ecef_x = struct.unpack('<d', bytes.fromhex(seg[10:26]))[0] 
        sat_pos_ecef_y = struct.unpack('<d', bytes.fromhex(seg[26:42]))[0] 
        sat_pos_ecef_z = struct.unpack('<d', bytes.fromhex(seg[42:58]))[0] 
        segments.append({
            ...
            'Sat_Pos_ECEF_X':         sat_pos_ecef_x,
            'Sat_Pos_ECEF_Y':         sat_pos_ecef_y,
            'Sat_Pos_ECEF_Z':         sat_pos_ecef_z
        })
    return segments 