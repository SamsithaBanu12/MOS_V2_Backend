...
def HEALTH_ADCS_QUAT_ERR_VEC(hex_str):
    ...
    for idx in range(count):
        ...
        quat_err_x = struct.unpack('<h', bytes.fromhex(seg[10:14]))[0] * 0.01
        quat_err_y = struct.unpack('<h', bytes.fromhex(seg[14:18]))[0] * 0.01
        quat_err_z = struct.unpack('<h', bytes.fromhex(seg[18:22]))[0] * 0.01
        segments.append({
            ...
            'Quaternion Error Q1':             quat_err_x,
            'Quaternion Error Q2':             quat_err_y,
            'Quaternion Error Q3':             quat_err_z
        })
    return segments
